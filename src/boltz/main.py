...
def _detect_decaf_checkpoint(
    ...
    decaf_args = {
        ...
        "rho": dpa.get("rho", 7),
        "coordinate_augmentation": dpa.get("coordinate_augmentation", True),
...
@click.option(
    ...
    type=float,
    help=(
        "Stochastic noise injection scale for the diffusion sampler. "
        "Set to 0.0 to disable noise injection and use a deterministic ODE sampler. "
        "Default is 0.605 for Boltz-1. If not provided, the default is used."
    ),
    default=None,
)
@click.option(
    "--noise_scale",
    type=float,
    help="Noise scale for the diffusion sampler. Default is 0.901 for Boltz-1, 1.003 for Boltz-2.",
    default=None,
)
@click.option(
    "--diffusion_rho",
    type=float,
    help="Rho parameter for the Karras noise schedule. Default is 8 for Boltz-1, 7 for Boltz-2.",
    default=None,
)
@click.option(
    "--write_full_pae",
...
@click.option(
    ...
    type=int,
    default=None,
    help="Override FK num_particles (default: 3 with --use_potentials).",
)
@click.option(
...
@click.option(
    ...
    default=1.0,
    type=float,
    help="Gamma-sampling parameter. 1.0=full SDE, 0.0=ODE, 0<γ<1=partial denoise+renoise. Default is 1.0.",
)
@click.option(
    "--use_mc_grad",
    is_flag=True,
    help="Use Weighted MC-GRAD sampler for Decaf sampling (requires --use_potentials).",
)
@click.option(
    "--mc_grad_particles",
    default=4,
    type=int,
    help="Number of MC-GRAD particles K. Default is 4.",
)
@click.option(
    "--mc_grad_snr_ratio",
    default=10.0,
    type=float,
    help="MC-GRAD SNR ratio lambda (sigma' = sqrt(lambda)*sigma_t). Default is 10.0.",
)
@click.option(
    "--no_mc_grad_gd",
    is_flag=True,
    help="Disable iterative GD guidance steps after MC-GRAD correction (pure mc_grad only).",
)
@click.option(
    "--use_mcts",
    is_flag=True,
    help="Use MCTS Full Rollout sampler for Decaf sampling (requires --use_potentials).",
)
@click.option(
    "--mcts_simulations",
    default=50,
    type=int,
    help="Number of MCTS simulations per sample. Default is 50.",
)
@click.option(
    "--mcts_children",
    default=4,
    type=int,
    help="Number of expansion children per MCTS node. Default is 4.",
)
@click.option(
    "--mcts_num_roots",
    default=0,
    type=int,
    help="Number of independent MCTS roots (best-of-N trees). 0 = use multiplicity. Default is 0.",
)
@click.option(
    "--model",
    default="boltz2",
...
def predict(  # noqa: C901, PLR0915, PLR0912
    ...
    diffusion_rho: Optional[float] = None,
    ...
    mc_grad_particles: int = 4,
    mc_grad_snr_ratio: float = 10.0,
    no_mc_grad_gd: bool = False,
    use_mcts: bool = False,
    mcts_simulations: int = 50,
    mcts_children: int = 4,
    mcts_num_roots: int = 0,
    ...
    if model == "boltz2":
        ...
        if noise_scale is not None:
            diffusion_params.noise_scale = noise_scale
        if diffusion_rho is not None:
            diffusion_params.rho = diffusion_rho
    ...
    if filtered_manifest.records:
        ...
        if fk_resampling_interval is not None:
        ...
        if is_decaf:
            # Let the checkpoint's own msa_args/pairformer_args/diffusion_process_args
            # take effect — only override what we must.
            load_kwargs["decaf_args"] = decaf_args
        else:
            load_kwargs["diffusion_process_args"] = asdict(diffusion_params)
            load_kwargs["pairformer_args"] = asdict(pairformer_args)
            load_kwargs["msa_args"] = asdict(msa_args)
        ...
        # Option A: monkey-patch sampler config onto loaded model
        if no_sde and is_decaf and hasattr(model_module, "decaf_head") and model_module.decaf_head is not None:
        ...
        # Option B: MC-GRAD sampler
        if use_mc_grad and is_decaf and hasattr(model_module, "decaf_head") and model_module.decaf_head is not None:
            from boltz.model.modules.decaf_sampler import MCGradSamplerConfig
            model_module.sampler_config = MCGradSamplerConfig(
                use_sde=True,
                sde_gamma=sde_gamma,
                fk_steering=use_potentials,
                physical_guidance_update=use_potentials,
            ...
            logging.getLogger(__name__).info(
                ...
                mc_grad_particles, mc_grad_snr_ratio,
            )

        # Option C: MCTS Full Rollout sampler
        if use_mcts and is_decaf and hasattr(model_module, "decaf_head") and model_module.decaf_head is not None:
            from boltz.model.modules.decaf_sampler import MCTSFullRolloutDecafSamplerConfig
            num_roots = mcts_num_roots if mcts_num_roots > 0 else 1
            model_module.sampler_config = MCTSFullRolloutDecafSamplerConfig(
                use_sde=True,
                sde_gamma=sde_gamma,
                fk_steering=use_potentials,
                physical_guidance_update=use_potentials,
            ...
            logging.getLogger(__name__).info(
                ...
                mcts_simulations, mcts_children, num_roots,
            )

        # Fallback: if sde_gamma or use_potentials changed but no special sampler mode,
