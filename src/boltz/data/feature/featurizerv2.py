...
def process_residue_constraint_features(data: Tokenized) -> dict[str, Tensor]:
    ...
    if residue_constraints is not None:
        ...
        double_bond_dihedral_constraints = getattr(
            residue_constraints, "double_bond_dihedral_constraints", None
        )
        ...
        if double_bond_dihedral_constraints is not None and len(double_bond_dihedral_constraints) > 0:
            double_bond_dihedral_index = torch.tensor(
                double_bond_dihedral_constraints["atom_idxs"].copy(), dtype=torch.long
            ).T
        else:
            double_bond_dihedral_index = torch.empty((4, 0), dtype=torch.long)
    else:
    ...
    return {
        ...
        "double_bond_dihedral_index": double_bond_dihedral_index,
    }
