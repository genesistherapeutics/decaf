...
class PositionalEmbedding(Module):
    """Sinusoidal positional embedding with geometrically spaced frequencies.

    Following the DDPM++/FastGen convention:
        freqs_k = 1 / max_positions^(k / (num_channels/2))
        output = [cos(x * freqs), sin(x * freqs)]

    Used for log-scaled time embeddings in flow map conditioning.
    Unlike ``FourierEmbedding`` (random Fourier features, cosine-only),
    this uses deterministic, geometrically spaced frequencies with both
    cos and sin, giving predictable multi-scale representation.
    """
    ...
    def forward(self, x):
        ...
        freqs = torch.arange(start=0, end=half_dim, dtype=torch.float32, device=x.device)
...
class DualTimeConditioning(Module):
    """Dual time conditioning layer for flow map learning.

    Follows FastGen conventions for time conditioning:
      1. Log-transform: t -> log(max(t, eps)) / 4
      2. Separate sinusoidal embeddings for t and r
      3. Concatenate + MLP instead of element-wise addition
      4. Deterministic sinusoidal positional embedding with geometrically
         spaced frequencies (cos + sin)

    The input ``times`` tensor is ``cat([t, r], dim=0)`` where t is the
    current flow time and r is the target time (both in [0, 1]).
    """
    ...
    def __init__(
        ...
        self.embed_t = PositionalEmbedding(dim_fourier)
        self.embed_r = PositionalEmbedding(dim_fourier)

        self.time_mlp = nn.Sequential(
            nn.Linear(2 * dim_fourier, 2 * token_s),
            nn.SiLU(),
            nn.Linear(2 * token_s, 2 * token_s),
            nn.SiLU(),
        )
    ...
    def forward(
        ...
        *,
        ...
        t, r = torch.chunk(times, 2, dim=0)

        s = torch.cat((s_trunk, s_inputs), dim=-1)
        s = self.single_embed(self.norm_single(s))

        t_log = t.clamp(min=1e-5).log() / 4
        r_log = r.clamp(min=1e-5).log() / 4

        t_emb = self.embed_t(t_log)
        r_emb = self.embed_r(r_log)

        time_emb = torch.cat([t_emb, r_emb], dim=-1)
        time_proj = self.time_mlp(time_emb)

        s = rearrange(time_proj, "b d -> b 1 d") + s
    ...
    @property
    def normed_fourier(self):
        """DualTimeConditioning does not produce normed_fourier; return None for API compat."""
        return None
