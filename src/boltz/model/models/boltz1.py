...
from boltz.model.modules.decaf import AtomDecaf
...
class Boltz1(LightningModule):
    ...
    def __init__(  # noqa: PLR0915, C901, PLR0912
        ...
        decaf_args: Optional[dict[str, Any]] = None,
    ) -> None:
        super().__init__()
        ...
        # Optional Decaf head (Decaf self-distillation)
        self.decaf_head = None
        if decaf_args is not None:
            decaf_score_args = {
                "token_z": token_z,
                "token_s": token_s,
                "atom_z": atom_z,
            ...
            self.decaf_head = AtomDecaf(
                score_model_args=decaf_score_args,
                compile_score=compile_structure,
                **{k: v for k, v in decaf_args.items() if k != "loss_weight"},
            )
            self.decaf_loss_weight = decaf_args.get("loss_weight", 1.0)
            logger.info(
                "Decaf head initialized (target_type=%s, sigma_min=%.4f, sigma_max=%.1f)",
                decaf_args.get("target_type"),
                decaf_args.get("sigma_min", 0.0),
                decaf_args.get("sigma_max", 0.0),
            )
    ...
    def forward(
        ...
        if self.training and self.structure_prediction_training:
            ...
            # Decaf head training (teacher = structure_module)
            if self.decaf_head is not None:
                decaf_out = self.decaf_head(
                    s_trunk=s,
                    z_trunk=z,
                    s_inputs=s_inputs,
                    feats=feats,
                    ...
                    multiplicity=multiplicity_diffusion_train,
                    training=True,
                    diffusion_module=self.structure_module,
                )
                dict_out["decaf_out"] = decaf_out

        if (not self.training) or self.confidence_prediction:
            # Use decaf_head for sampling when available, else structure_module
            sample_module = (
                self.decaf_head if self.decaf_head is not None else self.structure_module
            )
            if self.decaf_head is not None:
                sampler_config = getattr(self, "sampler_config", None)
                _sampler_name = (
                    "MCTSFullRolloutDecafSampler" if sampler_config and "mcts" in type(sampler_config).__name__.lower()
                    else "MCGradSampler" if sampler_config and "mc_grad" in type(sampler_config).__name__.lower()
                    else "DecafSampler"
                )
                logger.info("Sampling with %s (steps=%s, samples=%s)",
                            _sampler_name, num_sampling_steps, diffusion_samples)
                dict_out.update(
                ...
                logger.info("Sampling with standard AtomDiffusion (steps=%s, samples=%s)", num_sampling_steps, diffusion_samples)
                dict_out.update(
                    sample_module.sample(
                        s_trunk=s,
                        z_trunk=z,
    ...
    def training_step(self, batch: dict[str, Tensor], batch_idx: int) -> Tensor:
        ...
        # Decaf loss (optional)
        decaf_loss_value = torch.tensor(0.0, device=batch["token_index"].device)
        decaf_loss_dict = None
        if self.decaf_head is not None and "decaf_out" in out:
            try:
                decaf_loss_dict = self.decaf_head.compute_loss(
                    batch,
                    out["decaf_out"],
                    multiplicity=self.training_args.diffusion_multiplicity,
                )
                decaf_loss_value = decaf_loss_dict["loss"]
            except Exception as e:
                print(f"Skipping decaf loss for batch {batch_idx}: {e}")
        ...
        loss = (
            ...
            + getattr(self, "decaf_loss_weight", 0.0) * decaf_loss_value
        )
        # Log losses
        self.log("train/distogram_loss", disto_loss)
        self.log("train/diffusion_loss", diffusion_loss_dict["loss"])
        if self.decaf_head is not None:
            self.log("train/decaf_loss", decaf_loss_value)
            if decaf_loss_dict is not None:
                for k, v in decaf_loss_dict.get("loss_breakdown", {}).items():
                    self.log(f"train/decaf_{k}", v)
    ...
    def training_log(self):
        ...
        if self.decaf_head is not None:
            self.log(
                "train/grad_norm_decaf_head",
                self.gradient_norm(self.decaf_head),
                prog_bar=False,
            )
            self.log(
                "train/param_norm_decaf_head",
                self.parameter_norm(self.decaf_head),
                prog_bar=False,
            )
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> Any:
        try:
            ...
            if self.predict_args.get("write_full_pae", True) and "pae" in out:
                pred_dict["pae"] = out["pae"]
            if self.predict_args.get("write_full_pde", False) and "pde" in out:
